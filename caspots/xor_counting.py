...
class Propagator:
	...
	#literals = [init.solver_literal(atom.literal) for atom in init.symbolic_atoms.by_signature("supp",4) if abs(init.solver_literal(atom.literal)) != 1]	
	dnf = [init.solver_literal(atom.literal) for atom in init.symbolic_atoms.by_signature("dnf",2) ]#if abs(init.solver_literal(atom.literal)) != 1]
        clause = [init.solver_literal(atom.literal) for atom in init.symbolic_atoms.by_signature("clause",3) ]#if abs(init.solver_literal(atom.literal)) != 1] #
	
	literals = set(dnf + clause)
        if len(literals) > 0:
